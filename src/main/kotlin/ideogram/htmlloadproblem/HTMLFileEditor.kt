...
import ideogram.htmlloadproblem.LifecycleService
...
internal class HTMLFileEditor(private val project: Project,
                              private val file: LightVirtualFile,
                              request: HTMLEditorProvider.Request) : UserDataHolderBase(), FileEditor {
  ...
  private val htmlTabScope = LifecycleService.projectScope(project).childScope("HTMLFileEditor[${file.name}]")
  ...
  init {
    ...
    val requestHandler = object : CefRequestHandlerAdapter() {
      override fun onBeforeBrowse(browser: CefBrowser,
                                  ...
                                  request: CefRequest,
                                  userGesture: Boolean,
                                  isRedirect: Boolean): Boolean = if (userGesture) {
        navigating.set(true); browse(request.url); true
      } else false
    }

    contentPanel.jbCefClient.addRequestHandler(requestHandler, contentPanel.cefBrowser)

    contentPanel.jbCefClient.addLifeSpanHandler(object : CefLifeSpanHandlerAdapter() {

      override fun onBeforePopup(browser: CefBrowser,
                                 frame: CefFrame,
                                 targetUrl: String,
                                 targetFrameName: String?): Boolean {
        browse(targetUrl)
        return true
      }
    }, contentPanel.cefBrowser)
    ...
    if (queryHandler != null) {
      val config = CefMessageRouter.CefMessageRouterConfig(HTMLEditorProvider.JS_FUNCTION_NAME,
                                                           "${HTMLEditorProvider.JS_FUNCTION_NAME}Cancel")
      jsRouter = JBCefApp.getInstance().createMessageRouter(config)
      jsRouter!!.addHandler(object : CefMessageRouterHandlerAdapter() {
        override fun onQuery(browser: CefBrowser,
                             frame: CefFrame,
                             id: Long,
                             request: String?,
                             persistent: Boolean,
                             callback: CefQueryCallback): Boolean {
          htmlTabScope.launch {
            runCatching { queryHandler.query(id, request ?: "") }.onSuccess { callback.success(it) }
    ...
    contentPanel.jbCefClient.addDisplayHandler(object : CefDisplayHandlerAdapter() {
      override fun onStatusMessage(browser: CefBrowser, text: String) = StatusBar.Info.set(text, project)
    }, contentPanel.cefBrowser)
